...
| [smb-second-brain](./claude-skills/smb-second-brain) | `1.0` | Turn a folder of docs — transcripts, contracts, CIMs, notes — into an LLM-maintained wiki of linked markdown you can ask questions, with citations. Opens in Obsidian as-is; one script publishes it as a 3D knowledge graph site. No database, no embeddings. Karpathy's LLM-wiki model, five operations. **Claude Code for publishing; the rest works in claude.ai too.** |
